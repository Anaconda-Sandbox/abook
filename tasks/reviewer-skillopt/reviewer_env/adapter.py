"""SkillsBench `reviewer` adapter for SkillOpt (agentbook-local; library use).

Optimizes a code-review SKILL.md against planted-bug recall. Reflect + patch + selection
...
from .dataloader import SkillsBenchReviewerDataLoader
from .rollout import run_reviewer_batch

TASK_TYPES = ["code_review"]


class SkillsBenchReviewerAdapter(EnvAdapter):
    """SkillsBench reviewer slice — planted-bug recall, deterministic reward."""
    ...
    def __init__(
        ...
        self.dataloader = SkillsBenchReviewerDataLoader(
            split_dir=split_dir,
            data_path=data_path,
    ...
    def rollout(self, env_manager, skill_content: str, out_dir: str, **kwargs) -> list[dict]:
        items = env_manager  # list of instance dicts
        os.makedirs(out_dir, exist_ok=True)
        results_path = os.path.join(out_dir, "results.jsonl")
        if os.path.exists(results_path):  # resume support
            existing = []
