...
2. **Given** the inner loop is mid-run, **When** the host stops or interrupts the run, **Then** the inner loop halts cleanly with a partial result rather than losing all recorded state.
...
*(FR-007 removed — budget abstraction dropped for simplicity)*
...
- **Inner LLM**: the LLM running inside the optimizer (reflection / analyst) — thin intelligence, running inside the inner loop.
...
- **A-002**: The host agent has authority to choose which optimizer library is loaded into a session.
- **A-003**: LLM API credentials are provisioned in the kernel environment before the session starts.
