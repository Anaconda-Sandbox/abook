...
def build_cfg() -> dict:
    nested = {
        "model": {
            "optimizer_backend": "claude_chat", "target_backend": "claude_chat",
            "optimizer": "claude-sonnet-4-6", "target": "claude-sonnet-4-6",
        ...
        "train": {"num_epochs": 1, "train_size": 0, "batch_size": 6,
                  "accumulation": 1, "seed": 42},
        "gradient": {"minibatch_size": 3, "merge_batch_size": 3, "analyst_workers": 2,
                     "max_analyst_rounds": 2, "failure_only": False},
        "optimizer": {"learning_rate": 3, "min_learning_rate": 2, "lr_scheduler": "constant",
        ...
        "env": {"name": "skillsbench", "skill_init": SEED_SKILL, "split_mode": "split_dir",
                "split_dir": SPLIT_DIR, "split_seed": 42, "exec_timeout": 120,
                "workers": 4, "out_root": OUT_ROOT},
    }
...
def main() -> None:
    ...
    adapter = SkillsBenchReviewerAdapter(
        split_dir=SPLIT_DIR, split_mode="split_dir", workers=4,
        analyst_workers=2, minibatch_size=3, edit_budget=3, exec_timeout=120,
    )
